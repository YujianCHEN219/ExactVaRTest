...
ExactVaRTest implements dynamic programming algorithms (C++ backend with a pure-R fallback) that give the exact finite-sample distributions and p-values of Christoffersen’s (1998) independence (IND) and conditional-coverage (CC) tests for Value-at-Risk (VaR) exception series.
...
Exact LR distributions and p-values for any sample size n (≈ 2 000 in seconds).

C++ implementation (`Rcpp`) with automatic fallback to a pure-R reference version.

High-level helper `backtest_lr()` with printing: statistic, p-value, decision, and all parameters in one object.
